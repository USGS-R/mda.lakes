plot.fig8.GCB	<-	function(years){
	...
	plot_colors <- c("black", "firebrick", "grey80","grey20")
	
	png(filename = "../stratification_onset.png",
	    width = 3.14961, height = 4.25, units = "in", res=300)
	
	
	panels	<-	rbind(c(1,4),c(2,5),c(3,6))
	
	layout(panels)
	par(mai=c(.3, .3, .025, .025),mgp=c(.9,.18,0))
	
	# plot 1 & 4 are air temperatures
	plot.air(years[1],col=plot_colors[2],plt.rng.x,cex.ttl,cex.box,tck)
 	
	plot.exceed(years[1],col=plot_colors,plt.rng.x,cex.ttl,cex.box,tck)
	
	plot.strat(years[1],col=plot_colors,plt.rng.x,cex.ttl,cex.box,tck)
	
	plot.air(years[2],col=plot_colors[2],plt.rng.x,cex.ttl,cex.box,tck)
 	
	plot.exceed(years[2],col=plot_colors,plt.rng.x,cex.ttl,cex.box,tck)
	
	plot.strat(years[2],col=plot_colors,plt.rng.x,cex.ttl,cex.box,tck)

	dev.off()
}

plot.air	<-	function(year,col,plt.rng.x,cex.ttl,cex.box,tck){
	...
	axis(2,at=seq(-10, 50, 10),las=1, cex.axis=cex.box, tck=tck)
	axis(4,at=seq(-10, 50, 10),las=1, cex.axis=cex.box, tck=tck,labels=NA)
...
plot.exceed	<-	function(year,col,plt.rng.x,cex.ttl,cex.box,tck){
...
plot.strat	<-	function(year,col,plt.rng.x,cex.ttl,cex.box,tck){
	plot(c(NA,1),c(0,1), type="l", col=col[1], 
		axes=F, xlab="Day of year",
		ylim=c(0,100), xlim=plt.rng.x,
		ylab="Percentage of lakes (%)",
