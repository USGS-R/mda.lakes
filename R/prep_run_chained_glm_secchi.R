#'@title Prepare and run chained model with Kd scenario
#'
#'@param site_id
#'@param path 
#'@param kd_trend The percentage of Secchi increase per year. Positive is increasing secchi (e.g., 0.94 is 0.94% increasing secchi depth per year)
#'@param nml_args
#'
#'
#'@description 
#'Pulls in the required information to do a simple GLM 
#'run. Grabs driver data from Sciencbase and attribute data
#'from the internal package DB. 
#'
#'@examples
#'dir.create('~/test')
#'	dir.create(to_run[i])
#'	
#'	prep_run_chained_glm(to_run[i], path=to_run[i], 
#'									 start=as.POSIXct('1979-01-01'), 
#'									 end=as.POSIXct('2011-12-31'))
#'									 
#'
#'@import glmtools
#'@import GLMr
#'@export
prep_run_chained_glm_secchi <- function(site_id, path, start=as.POSIXct('2008-04-01'), 
																				end=as.POSIXct('2008-06-01'), secchi_trend = 0, 
																				nml_args=NULL){
	
	
	nml_obj = populate_base_lake_nml(site_id)
	
	
	#finally, change supplied nml
	if(!is.null(nml_args)){
		nml_obj = set_nml(nml_obj, arg_list=nml_args)
	}
	
	#Write nml file
	#nml_out_path = file.path(path, "glm.nml")
	#write_nml(nml_obj, nml_out_path)
	
	#iterate over all years
	start_year = as.POSIXlt(start)$year+1900
	end_year = as.POSIXlt(end)$year+1900
	years = start_year:end_year
	out_vals = rep(NA, length(years))
	
	for(i in 1:length(years)){
		year = years[i]
		
		off_date = getIceOff(site_id, year)
		on_date = getIceOn(site_id, year)
		
		if(is.na(off_date) | is.na(on_date)){
			warning(site_id, ' does not have ice ON and OFF dates for ', year, '\nSkipping...')
			next
		}
		
		kd = getScenarioKd(site_id, year, year.1 = start_year, year.2 = end_year, trend = secchi_trend,
											 default.if.null = FALSE)
		
		if(is.null(kd)){
			stop('No kd value for:', site_id)
		}
		
		
		#start stop
		nml_obj = set_nml(nml_obj, 'start', format(as.POSIXct(off_date), '%Y-%m-%d %H:%M:%S'))
		nml_obj = set_nml(nml_obj, 'stop', format(as.POSIXct(on_date), '%Y-%m-%d %H:%M:%S'))
		nml_obj = set_nml(nml_obj, 'out_fn', paste0('output', year))
		nml_obj = set_nml(nml_obj, 'Kw', kd)
		
		nml_out_path = file.path(path, "glm2.nml")
		write_nml(nml_obj, nml_out_path)
		out_vals[i] = run_glm(path)
	}
	
	all_kds = getScenarioKd(site_id, years, year.1 = start_year, year.2 = end_year, trend = secchi_trend,
													default.if.null = FALSE)
	
	output = data.frame(year=years, kd=all_kds, out_val=out_vals)
	
	return(output)
}
