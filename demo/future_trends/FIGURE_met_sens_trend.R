...
for(i in 1:length(vars)){
  ...
  driver_slopes = sens_seasonal_site(year(driver$time), data = driver[,vars[i]], season_i = yday(driver$time))
  
  season_slopes = ddply(driver_slopes, 'season_i', function(df){data.frame(med=median(df$slopes))})
  
  png(paste0('~/season.sens.', vars[i],'.png'), res=300, width=2000, height=1500)
