plot.fig8.GCB	<-	function(years){
  ...
  panels = matrix(c(1,2,3),ncol=3)
	...
	plot.exceed(years[1],col=plot_colors,plt.rng.x,cex.ttl,cex.box,tck,label='(a)')

	plot.exceed(years[2],col=plot_colors,plt.rng.x,cex.ttl,cex.box,tck,label='(b)')
  
	plot.exceed(years[3],col='red',plt.rng.x,cex.ttl,cex.box,tck,label='(c)')
...
plot.exceed	<-	function(year,col,plt.rng.x,cex.ttl,cex.box,tck,label){
  ...
  med_x <- get_median_exceed(exceed$line.3)
  
  
  
	lines(exceed$x,exceed$line.3,type="l", lwd=3,
		col=col[1])
